  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const isMixMenuNoneSub = computed(() => {
        const mixMenu = menuSetting.value.mixMenu;
      ...
      const getChangeStyle = computed(() => {
        ...
        let lenNum =
          navMode.value === 'horizontal' || !isMixMenuNoneSub.value
            ? '0px'
            : collapsed
            ? `${appearanceStore.layoutConfig.collapsedMenuWidth}px`
            : `${appearanceStore.layoutConfig.menuWidth}px`;
...
<style lang="less" scoped>
  ...
  .tabs-view-fix {
    position: fixed;
    z-index: 5;
    padding: 6px var(--app-content-padding);
