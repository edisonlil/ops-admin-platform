...
`llm_runtime` 是系统内统一的 LLM Gateway / Model Router bounded context。它负责管理模型供应商、模型目录、业务任务注册、路由策略、运行时调用和调用观测。业务领域不直接维护 API Key、供应商、模型名或 fallback 规则。
...
- 提供统一调用入口，例如 `llm_gateway.generate(...)`。
...
- 具体业务 prompt 的构造。
- 业务结果解析和业务规则判断。
- 直接依赖任一业务领域的 infrastructure。
- 把模型配置长期维护在 `.json` 文件中。

业务领域负责：

- 声明自己需要的 LLM 任务。
- 在业务应用服务中调用 `llm_runtime` 的 application port。
- 维护本领域 README，说明任务清单、调用方式、依赖关系和集成约束。

## 数据库优先原则

模型配置的权威来源是数据库。后台页面、运行时调用和测试夹具都应优先围绕数据库模型设计。

`.json` 配置只允许作为历史兼容或本地临时调试入口，不作为长期维护方式。新增供应商、模型、任务和路由策略时，应优先增加 bounded context 自有的 DDL、repository/use case 和 HTTP API。
...
Model 保存模型层信息：`model_key`、`provider_key`、供应商真实模型名、能力标签、上下文长度、是否支持 JSON、是否支持 reasoning 输出、启用状态。
...
- `route_key` 唯一。
- `route_key` 可以是精确 task，也可以是兜底 key，例如 `function_point.default`。
- 第一版只实现 `priority` 策略。
...
- 单个 entry 可禁用。
- `response_format` 第一版支持 `text` 和 `json`。
- 只有超时、限流、5xx、供应商临时错误等可恢复错误触发 fallback。
...
- 日志不保存完整 prompt 和模型输出，避免敏感数据泄漏。
- 如需调试内容，应设计单独的受控 trace 存储和脱敏策略。
...
```text
1. 根据 task_key 生成候选 route_key。
2. 查找第一个 enabled 的 llm_routing_policies。
3. 读取 policy 下 enabled 的 entries，按 priority 排序。
4. 对每个 entry 解析 model 和 provider。
5. 创建供应商客户端并调用模型。
6. 成功后返回 LLMResponse。
7. 可恢复错误时记录日志并尝试下一个 entry。
8. 全部失败后抛出稳定的运行时错误。
```
...
兼容期内，已有 `build_llm_client(role=...)` 可以保留，但新代码应逐步迁移到 Gateway。旧的 `role` 参数应映射到新的 `task_key`。
...
管理类 HTTP 响应必须使用统一 envelope。列表接口必须把数据放在 `data.items` 和 `data.pagination` 下。

OpenAI 兼容接口用于外部系统和后台模型调试页直接调用已配置模型：

- `GET /api/llm/openai/v1/models` 返回 OpenAI 风格的模型列表，包含已启用的 `model_key` 和已启用的 `route_key`。
- `POST /api/llm/openai/v1/chat/completions` 请求体兼容 OpenAI Chat Completions。`model` 可以传数据库中的 `model_key`/供应商真实 `model_name`，也可以传 `route_key`/业务 `task_key`。
- 当 `model` 命中模型目录时，调用该模型所属 provider；当 `model` 未命中模型目录时，按路由策略解析并支持 fallback。
- 兼容接口需要登录态或 `X-API-Key`，但响应形状保持 OpenAI 风格，不包统一 envelope，方便 SDK 和外部调用方接入。
...
- 外部依赖：依赖哪些其他 context 的 application port 或 domain event。
- LLM 集成：声明哪些 task key、prompt 构造位置、返回解析位置、错误处理方式。
- 数据库资源：本领域拥有的表、DDL 和 seed 位置。
...
- 保留现有 `/api/llm-config` 和 `llm_configs`，作为默认模型兼容入口。
- 新增 provider、model、task、routing policy 表。
- 把现有单配置迁移为 `default` route。

第二阶段：

- 新增 Gateway 调用入口。
- `function_point` 的 recall/rank/eval 从旧 role 迁移为 task key。
- 后台页面改为维护供应商、模型和路由策略。

第三阶段：

- 增加调用日志页面。
- 增加 fallback 观测、禁用 entry、模型健康状态。
- 逐步废弃 `.json` 作为运行时配置来源。

## 实现约束

- `domain` 不依赖 FastAPI、数据库客户端、HTTP 客户端或供应商 SDK。
- `application` 定义用例、路由解析、事务边界和端口。
- `infrastructure` 实现数据库 repository 和供应商客户端适配器。
- `interfaces/http` 只负责 DTO、鉴权、调用 application service 和 envelope 映射。
- 跨领域调用只能依赖 application port 或 domain event，不直接导入其他 context 的 infrastructure。
- 新增表必须同时提供 SQLite 和 PostgreSQL DDL。
- 新增运行时行为必须补充 application use case 测试和架构边界测试。
